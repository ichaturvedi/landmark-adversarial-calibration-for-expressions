...
def train_val():
	...
	dataFeeder = data.domainTransferLoader('happycn_neutral')
	train_loader = torch.utils.data.DataLoader(dataFeeder, batch_size=1, shuffle=True,
											   num_workers=1, pin_memory=True)

	criterion = nn.BCEWithLogitsLoss().cuda()

	optimizerD = torch.optim.Adam(discriminatorD.parameters(), lr=0.000001)
	optimizerA = torch.optim.Adam(discriminatorA.parameters(), lr=0.000001)
	optimizerG = torch.optim.Adam(generator.parameters(), lr=0.000001)

	generator.train()
	discriminatorD.train()
	discriminatorA.train()
	os.system("rm samplestest/*")	
	...
	for epoch in range(1):
		...
		for i, (image1, image2, image3) in enumerate(train_loader):
			...
			if((epoch+1) % 1) == 0:
                                ...
                                torchvision.utils.save_image((output_pair+1)/2, 'samplestest/'+str(i+1)+'.jpg')
                                torch.save(generator,'saved_models/generator.pt')
                                torch.save(discriminatorA,'saved_models/discriminatora.pt')
                                torch.save(discriminatorD,'saved_models/discriminatord.pt')


if __name__ == '__main__':
	os.system('mkdir -p samplestest')
	train_val()
